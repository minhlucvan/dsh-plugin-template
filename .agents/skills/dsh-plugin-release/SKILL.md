...
Confirm the explicitly selected package name, version, description, keywords, license, repository metadata, Node engine, package manager, Cordis plugin id, invariant package name, bundle rows, README examples, and lockfile all describe the same package. The package name may be scoped or unscoped; compare the exact name rather than requiring the template's `@scope/dsh-` prefix.
